...
class MolGen(nn.Module):
    ...
    def __init__(self, data, clf_path, hidden_dim=128, lr=1e-3, device='cpu'):
        ...
        self.classfier = pi.load(open(clf_path, 'rb'))
    ...
    #
    #     descriptors_set = np.empty((0, num_descriptors), float)
    #     descriptor_names = [name + '.1' for name in descriptor_names]
    #
    #     for smiles_mol in pack:
    #         if Chem.MolFromSmiles(smiles_mol) is None:
    #             pack.remove(smiles_mol)
    #             continue
    ...
    #         descriptors_set = np.append(descriptors_set, descriptors, axis=0)
    #         gen_table = pd.DataFrame(descriptors_set, columns=descriptor_names)
    #
    #     x_gen_for_clf = drug_table.merge(gen_table, how='cross')
    #
    #     clf_predicts = clf.predict(x_gen_for_clf)
    #     clf_desired_values = [desired_value] * len(clf_predicts)
    #
    #     clf_loss = (np.subtract(clf_desired_values,clf_predicts)) ** 2
    #
    #     return(clf_loss)
        #Nina
    ...
    def train_step(self, x):
        ...
        pack = self.generate_n(100)
        print('generator_loss', torch.stack(generator_loss).mean()) # вывод первого слагаемого - тензор?? - иногда отрицательный??
        
        generator_loss = torch.stack(generator_loss).mean() + np.mean(self.clf(pack)) - \
            sum(entropies) * 0.01 / batch_size
        
        print('clf loss', np.mean(self.clf(pack))) # вывод второго слагаемого
        print('generator_loss_result', generator_loss) # вывод результата
        # Nina
    ...
    def generate_n(self, n):
        ...
        x = self.generator(z)['x'].to(self.device)

        lenghts = (x > 0).sum(1)

        # l - 1 because we exclude end tokens
        return [self.get_mapped(x[:l-1]) for x, l in zip(x, lenghts)]
