...
class Generator(nn.Module):
    ...
    def forward(self, z, max_len=100):
        ...
        # start of sequence
        starts = torch.full(
            size=(batch_size,), fill_value=self.start_token, device=torch.device('cpu')).long()
        ...
        end_pos = (x == self.end_token).float().argmax(dim=1).to(torch.device('cpu'))
...
class RecurrentDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        starts = torch.full(
            size=(batch_size, 1), fill_value=self.start_token, device=torch.device('cpu')).long()
